...
## ⚠️ 撤回：val14 遮挡 gap 尚未成立（2026-09-02 更正）

原先此处记录"遮挡 gap −1.67，3.1 个标准误"，**该结论已撤回**。

### 直接证据：同配置重跑差 0.78 分

用**完全相同的配置**重跑一次 baseline：

| 对比 | 均值 | 变化的场景 | 从满分跌到 0 |
|---|---|---|---|
| **复制 vs 原 baseline（同配置）** | **−0.78** | **621/1118** | **7** |
| 遮挡 vs 原 baseline | −1.67 | 695 | 12 |
| 遮挡 vs 复制 baseline | **−0.90** | 700 | 12 |
| 随机扣留 vs 原 baseline | **−17.11** | 781 | **145** |

**遮挡的效应和噪声同量级**：换个 baseline 参照就从 −1.67 缩到 −0.90，而噪声本身是 −0.78。
"12 个场景归零"对比"噪声自带 7 个"，也分不开。

### 两个被推翻的推断

**"423 个场景变化精确为 0.00，所以流水线确定"**——错。那 423 个是遮挡没扣留任何东西的
场景，稳定性不能外推。直测下来 **621/1118 会变**。

**"3.1 个标准误"**——错。它把配对差值当成无噪声观测，而配对本身带 0.78 分噪声，分母是错的。

教训与"单向验证不算验证"同类：**先做能证伪的对照，再宣布结果。** 这已是本项目第三次。

### 根因与修复

`flow_planner/model/flow_planner_model/flow_planner.py:183`：

```python
x_init = torch.randn((B, self.action_num, ...), device=self.device)
```

流匹配推理每次从全局 RNG 抽初始噪声。本地实测：**同一进程内连跑同一场景两次，轨迹差 0.288 米**。
全量下 ray 复用 worker、调度顺序决定各自的 RNG 状态，于是 621/1118 不同。

`smart/nuplan/seeded_planner.py` 在每次 planner 调用前按**步号**播种。按步而非按场景是有意的：
两个条件在第 k 步抽到同一份噪声（common random numbers，配对比较的标准降方差手段）。

**通过官方 runner 验证**：两轮 total 与逐场景分数**逐位相同**（62.707119449）。

⚠️ 这是对 planner 的干预，任何结果都必须声明。它不改变 planner 算什么，只固定它在等概率
样本中取哪一个。改 `seed` 可得另一个同样可复现的样本——**跑几个种子才能把真效应和采样运气分开**。

### 仍然成立的

**随机扣留对照**：−17.11 分、145 个场景归零，远在噪声之外。所以这条不受影响——
**损害取决于扣留了谁而非多少**，同等数量下随机扣留的破坏是遮挡的十几倍，
说明遮挡挑走的恰恰是最不要紧的目标。

### 待办

- 用 `SEED=0` 重跑 baseline / occluded / random 三轮
- 至少再跑一个种子，量化采样运气的贡献
- 在此之前，**遮挡的 gap 不得引用**

---

## （已作废，保留存档）val14 全量，遮挡 gap（2026-09-01）
...
⚠️ **`following_lane_with_lead` 的 −19.85 已查证，不可按字面使用。**

15 个场景里只有 3 个出事，各从 100 掉到 0（3×100/15 = 20，正好是那个均值）。
三个都是同一个门判零：`no_ego_at_fault_collisions`，而可行驶区域 / 行驶方向 /
是否前进 / 沿专家路线进度**全部保持 1.0**——ego 正常行驶然后撞了。

**但碰撞对象是它看得见的。** `scripts/collision_forensics.py` 用 devkit 自己的
`in_collision` 查证，三个全部是 **HANDED OVER**（碰撞时刻该车已交付给 planner，
baseline 里也在）：

| 场景 | 碰撞时刻 | ego 速度 | 该目标 | 两次仿真 ego 位置差 |
|---|---|---|---|---|
| 7426524f502552e0 | t=8.3s | 4.21 m/s | 已交付 | 0.96 m |
| 7f52eb0df56b536d | t=4.1s | 3.98 m/s | 已交付 | 2.72 m |
| 89f3b2c5f2755984 | t=6.8s | 8.29 m/s | 已交付 | 1.86 m |

**曾据"其余指标都是 1.0"推断是"撞了看不见的车"，直接查证否定了这个推断。**
实际链条是：遮挡改变了更早的决策 → 轨迹累积偏移 1–3 m → 撞上一辆可见的车。

两种读法未能区分：**(a) 真实的间接因果**（遮挡起因，链条成立）；
**(b) planner 脆弱性**（一两米扰动即翻转边缘结果，则任何同量级扰动都会）。

**所需对照**：用非遮挡方式施加同量级扰动（随机丢弃远处 agent、或位置加噪声），
看灾难性翻转的比例是否相当。**在这个对照做出来之前，这三个场景不能作为遮挡致因的证据。**

⚠️ 但这不推翻聚合结果：−1.67 的 gap、按类型的模式、以及两个近百样本的阴性对照
（+0.00 / +0.01）依然成立——**纯粹的普遍脆弱性不会让阴性对照恰好为零**。
...
## PDM-Closed 全量结果：不是免疫，而且是目前最干净的证据（2026-09-03）

**先更正**：此前基于 mini 上 4 个场景写下"PDM-Closed 对遮挡完全免疫"，**全量数据推翻了它**。
那 4 个场景恰好都没受影响。**又一次把单场景数字当结论**——本项目第四次同类错误。
...
### 为什么这比 Flow Planner 的结果更硬

**PDM-Closed 是确定性的**——997/1118 个场景变化精确为 0.00，有采样随机性的 planner 做不到。
**所以它没有噪声底，那 121 个变化全部是遮挡的因果结果，不需要显著性检验。**

对照之下 Flow Planner 的 −1.67 至今悬着，因为它的噪声底是 0.78（见上文撤回）。

### 顺带验证了 harness

**PDM-Closed baseline 92.84 对上已发表的 93。** 这说明流水线本身没问题，
Flow Planner 那 2.5 分缺口是它自己的事（CFG 默认关闭，或发布权重非论文所用）。

### 两个从满分归零的场景（已存动图）

| token | 类型 |
|---|---|
| `04c0e32756d351e2` | near_multiple_vehicles |
| `133baa14fb5457ac` | stationary_in_traffic |

`gifs/pdm_closed/*.gif`（不入库）。第一个的画面显示 ego 夹在车队中，
`seen 31 / withheld 9`，被扣留的都是**同车道更远处、被中间车辆挡住的**——密集车队制造的遮挡。

次差的几个：`c30b795fb1025200`(−88.8, traversing_pickup_dropoff)、
`6d4ba74aba6d5722`(−68.8, waiting_for_pedestrian_to_cross)、
`55b7c9f3fc3753e9`(−68.8, starting_right_turn)。

### 现在的格局

| planner | 类型 | 确定性 | 遮挡 gap |
|---|---|---|---|
| IDM | 规则，只看前车 | 是 | **0.00** |
| **PDM-Closed** | 规则，全场，冠军 | **是** | **−0.38（121 场景变化，因果确凿）** |
| Flow Planner | 学习型 | 否（已加播种） | 待用 SEED+CFG 重测 |

**分界线不是"规则 vs 学习"**：PDM-Closed 是规则型却会受影响。更可能是**决策是否依赖自身路径走廊之外的目标**，而 PDM-Closed 的候选轨迹打分会考察走廊附近，所以有少量场景中招。
