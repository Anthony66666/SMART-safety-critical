...
**SMART 对中途进场的车没有任何歧视——是我们喂的那一行有洞。** tokenizer 从**固定锚点**
（第 0、5、10 步）构建运动 token。原有车 96-97% 三锚点齐全，按日志位置注入的新车只有 **26-30%**。
锚点缺失 → token 退化 → 词表只覆盖前向运动 → 横着开、倒着开。而且从日志回填历史在逻辑上不可能：
一辆车之所以"新出现"，恰恰因为日志之前没有它。要**合成**历史（消融：等速重构掉 0.7 分，带洞掉 5.5 分）。

**但注入日志位姿本身就是设计错误，修症状修不掉。** 日志世界和仿真世界从第 0 帧就分道扬镳，
SMART 的输出是同一张地图上**另一份**合理交通样本。把日志里的车连位置、朝向、速度一起塞进去，
就是混样：70% 的到达者落在别的仿真车 3 m 内。合成历史、不丢无 plan 的车、圆心距碰撞检查全是缓解。
正确做法是**只借日志的入口和时机，位姿由仿真世界定**——见 `_admit_entering_agents` 的六条规则
（吸附车道中心线、沿车道回溯历史、车道外的停放车回放、真实框相交检查含 ego 自身、由远到近逐辆）。
**曾经有一条"外围 40 m 才收"，已删。** 它把近处出现的车送去回放，而回放的车走专家的日志轨迹、
**不对 planner 反应**——恰恰把最危险的那批车变成了非反应式的。防叠车的活由真实框相交检查干，
去掉门槛后叠车 0/27。
官方 `IDMAgents` 做的正是吸附 + 碰撞检查，只是它根本不注入。改后新进车横开 2.8%、倒开 0、越界 0、
重叠 2/30。**三个参考实现（官方 IDM、devkit `AbstractMLAgents`、Bosch）都不中途加车**，
基类里留着 `TODO: consider agents appearing in the future`。

**行人 open-loop 回放，不是删掉。** 官方 IDM 的 `open_loop_detections_types` 就含 PEDESTRIAN。
回放把"从遮挡物后走出"这个动作按日志原样保留——比模型编的更真。行人仍进模型输入让车让路，
只是自己的位姿走日志。代价是不对 ego 反应，日志里 ego 是专家本来就让了。
而且按六条规则行人根本进不来（不走车道、到达时常静止、从遮挡物后突然出现），模型驱动它们只会把乱开带回来。

**现在所有到达的车都是模型驱动的**：27/27、6/6。只有车道外的停放车走回放。
路口等红灯的车（到达时静止但在车道上）也交给模型，合成历史是 11 帧原地不动——对等待的车这是真实的，
19 辆里 12 辆纹丝不动，其余只动一个 token 量子。

**统计横开率必须按移动帧加权，并忽略亚 token 位移。** 去掉门槛后横开率从 2.8% 升到 7.7%，
按到达速度拆开：到达时在动的车 70 帧、横开 2.9%（没变）；到达时静止的 19 辆共 8 个移动帧，
每辆一次 0.2 m 的微动，用位移方向对朝向算角度毫无意义，却在总量里和真实行驶等权。
**看到某个比率跳变，先看样本数。**

**统计越界率时必须只算车辆。** 之前报的"原有车 36-62% 越界"是把行人算成 agent 了——行人在人行道上。
只算车辆，原有车从头到尾都是 0.0%。那个数字从来不是驾驶缺陷，是测量错误。
同理 `anchors.py` 把回放的行人上下文行当成"新车"，剩下的 15% 锚点缺失就是它们，不驱动所以无害。
