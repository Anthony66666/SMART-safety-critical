...
（外围 40 m 才收、吸附车道中心线、沿车道回溯历史、停放车回放、真实框相交检查、由远到近逐辆）。
...
**大多数到达者是回放的，不是驱动的**：23/30、4/6。规则 1、2、4 把感知突然发现的、车道外的、
停放的都送去回放。所以中途交通是**回放为主、模型驱动为辅**，和官方 IDM 对全部物体的做法一致。
别把 "reactive" 读成"全部"。调 `ADMIT_MIN_DISTANCE_M` 能多驱动一些，但会把重叠带回来。
